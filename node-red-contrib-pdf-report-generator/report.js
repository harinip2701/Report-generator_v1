...
// locate the screenshots location 
        const screenshotPath = path.join('./node-red-contrib-pdf-report-generator/report', `screenshot_${i}.png`);
...
async function downloadImage(url, outputPath) {
  ...
  const outputImagePath = './node-red-contrib-pdf-report-generator/report/clientlogo.png';
...
async function generatePDFfromHTML(htmlContent, filename) {
    const fullPath = path.join('./node-red-contrib-pdf-report-generator/report', filename);
...
(async () => {
    ...
    for (let i = 0; i < urls.length; i++) {
        const imageData = await getImageBase64(`./node-red-contrib-pdf-report-generator/report/screenshot_${i}.png`);
        base64ImageData.push(imageData);
    }
    //placed our client logo and give a location where that logo image is located
    const clientlogos = await getImageBase64('./node-red-contrib-pdf-report-generator/report/clientlogo.png');
   
    //placed our logo and give a location where that logo image is located
    const iplon = await getImageBase64('./node-red-contrib-pdf-report-generator/report/iplon.png');
...
async function sendEmail() {
    try {
        const pdfFilePath = './node-red-contrib-pdf-report-generator/report/report.pdf';
        // Read the PDF file
        const pdfAttachment = {
            filename: 'report.pdf',
...
delaytime2();
    ...
    RED.nodes.registerType("PDF-report-generator",iplonReport);
}
